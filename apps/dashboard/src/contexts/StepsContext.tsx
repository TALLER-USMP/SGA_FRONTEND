...
import { useStepper } from "../hooks/useSteps";
import { cn } from "../lib/utils";

export const StepsContext = React.createContext({} as ReturnType<typeof useStepper>);
export const useSteps = () => React.useContext(StepsContext);

const StepsProvider: React.FC<React.PropsWithChildren<{ totalSteps: number }>> =
  ({ children, totalSteps }) => {

    const stepper = useStepper({ totalSteps });
    const steps = Array.from({ length: totalSteps }, (_, i) => i + 1);
    const renderedSteps = React.Children.toArray(children).filter(
